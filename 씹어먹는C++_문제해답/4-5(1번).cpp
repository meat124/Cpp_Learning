...
MyString &MyString::erase(int loc, int num)
{
    ...
    if (num < 0 || loc < 0 || loc > string_length)
        return *this;
    if (num > string_length) // 만약 지우려는 문자열의 길이가 실제 문자열의 길이보다 길다면
    {
        std::cout << "실제 문자열 길이보다 삭제하려는 길이가 더 깁니다." << std::endl;
        return *this;
    }
...
int MyString::compare(const MyString &str) const
{
    ...
    // 1이 나오면 *this 가 사전적으로 뒤에 있다는 의미이다.
    // 0 이 나오면 같다는 의미이다.
    // -1 이 나오면 str이 사전적으로 뒤에 있다는 의미이다.
    ...
    // 여기까지 통과했다면 만약 서로 문자열 길이가 같다면 같은 문자열이다.

    if (string_length == str.string_length)
        return 0;

    // 만약 문자열 길이가 (*this)가 더 길다면 사전순으로 더 뒤에 존재한다.
